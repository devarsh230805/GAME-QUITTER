  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => toggleChallenge(item.id)} style={({ pressed }) => [styles.row, pressed && { opacity: 0.8 }]}>
            <View style={[styles.checkbox, item.completed && styles.checkboxOn]}>
              {item.completed && <Text style={styles.checkmark}>✓</Text>}
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.itemTitle}>{item.title}</Text>
              <Text style={styles.itemMeta}>{item.completed ? 'Completed' : `+${item.reward} pts`}</Text>
            </View>
          </Pressable>
        )}
      />
