...
export default function OnboardingQuiz() {
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={styles.cta} onPress={() => setIdx((i) => (i + 1) % steps.length)}>
        <Text style={styles.ctaText}>{idx === steps.length - 1 ? 'Finish' : 'Next'}</Text>
      </Pressable>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  cta: { backgroundColor: colors.primary, paddingVertical: spacing.md, paddingHorizontal: spacing.xl, borderRadius: radii.pill, alignSelf: 'flex-start' },
